...
4. To create a docker image we need RHEL subscription (since we use registry.access.redhat.com/ubi8 image).
   ...
   In OpenShift environment, subscription is taken from host itself, however in case it is not taken from host
   we need to add below line in Dockerfile before 'yum install' step. For reference uncomment lines related
   to SUBS_USER and SUBS_PASS
   
    ```dockerfile
	RUN subscription-manager register --username ${SUBS_USER} --password ${SUBS_PASS} --auto-attach
    ```	
   We need to pass `SUBS_USER` and `SUBS_PASS` as build-args to docker build as mentioned in step 7
   
   **Note:** In case we are not using RHEL we can change base 
   image to centos instead of ubi8 and remove
         subscription-manager lines in Dockerfile. Mainly below lines
    ```dockerfile
	#ARG SUBS_USER
	#ARG SUBS_PASS
	#RUN subscription-manager register --username ${SUBS_USER} --password ${SUBS_PASS} --auto-attach
...
6. In case **git clone** for openafs repository does not work, follow below steps
	-  clone the openafs repository (git@github.com:openafs/openafs.git) on host
