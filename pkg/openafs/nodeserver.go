...
func (ns *nodeServer) NodePublishVolume(ctx context.Context, req *csi.NodePublishVolumeRequest) (*csi.NodePublishVolumeResponse, error) {
	...
	glog.Infof("Target Path %s", targetPath)
	executor := utilexec.New()
	volIdd := req.GetVolumeId()
	glog.Infof("Volume Id [%v]", volIdd)
	splitVolId := strings.Split(volIdd, "#")
	var mntPath string
	if (len(splitVolId) > 1) {
		mntPath = splitVolId[1]
	} else {
		mntPath = volIdd
	}
	   ...
	   glog.Infof("Failed to mount %v:%v", mntPath, targetPath)
           return &csi.NodePublishVolumeResponse{}, fmt.Errorf("failed to create link %v, %v", err, string(out))
        }
	glog.Infof("Mount %s on %s Successful", mntPath, targetPath)
...
func (ns *nodeServer) NodeUnpublishVolume(ctx context.Context, req *csi.NodeUnpublishVolumeRequest) (*csi.NodeUnpublishVolumeResponse, error) {
	...
	targetPath := req.GetTargetPath()
        ...
        glog.Infof("In Unpublish Target Path %s", targetPath)
        executor := utilexec.New()
        out, err := executor.Command("/bin/umount", targetPath).CombinedOutput()
        if err != nil {
